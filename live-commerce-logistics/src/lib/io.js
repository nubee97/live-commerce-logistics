...
//   OnlinePrice: r.livePrice || r.onlinePrice,
// }));
...
//   XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(eventProductsSheet), "Event Products");
//   XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(giftsSheet), "Gifts");
//   XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(aliasSheet), "Alias Mapping");

//   XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(brands), "Brands");
//   XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(legacyMain), "MainProducts");
//   XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(legacySets), "SetProducts");
//   XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(legacyComps), "SetComponents");
...
//     brandCode: String(r["Brand Code"] || "").trim(),
//     brandName: String(r["Brand Name"] || "").trim(),
//     productCode: String(r["Product Code"] || "").trim(),
//     sku: String(r["SKU"] || "").trim(),
//     productName: String(r["Official Product Name"] || "").trim(),
//     productImage: String(r["Product Image"] || "").trim(),
//     advantage: String(r["Advantage"] || "").trim(),
//     stock: Number(r["Stock"] || 0),
//     supplyPrice: Number(r["공급가"] || r["SupplyPrice"] || 0),
//     consumerPrice: Number(r["소비자가"] || r["ConsumerPrice"] || 0),
//     lowestPrice: Number(r["최저가"] || r["LowestPrice"] || 0),
//     livePrice: Number(r["라이브판매가"] || r["LivePrice"] || 0),

//     active: toBool(r["Active"], true),
//   }));

//   const eventProductsSheet = readSheet(wb, ["Event Products"]).map((r) => ({
//     eventSku: String(r["eventSKU"] || r["EventSKU"] || "").trim(),
//     eventCode: String(r["eventCode"] || r["EventCode"] || "").trim(),
//     productName: String(r["productName"] || r["ProductName"] || "").trim(),
//     productImage: String(r["image"] || r["ProductImage"] || "").trim(),
//     supplyPrice: Number(r["supplyPrice"] || r["SupplyPrice"] || 0),
//     salePrice: Number(r["salePrice"] || r["SalePrice"] || 0),
//     consumerPrice: Number(r["consumerPrice"] || r["ConsumerPrice"] || 0),
//     active: toBool(r["active"] ?? r["Active"], true),
//   }));

//   const aliasMappingSheet = readSheet(wb, ["Alias Mapping", "Aliases"]).map((r) => ({
//     id: newId(),
//     aliasName: String(r["AliasName"] || "").trim(),
//     targetType: String(r["TargetType"] || "PRODUCT").trim(),
//     targetSku: String(r["TargetSKU"] || "").trim(),
//     officialName: String(r["OfficialName"] || "").trim(),
//     active: toBool(r["Active"], true),
//   })).filter((r) => r.aliasName && r.targetSku);

//   const giftsSheet = readSheet(wb, ["Gifts"]).map((r) => ({
//     id: `g_${Math.random().toString(16).slice(2)}`,
//     giftName: String(r["GiftName"] || r["giftName"] || "").trim(),
//     giftCode: String(r["GiftCode"] || r["giftCode"] || "").trim(),
//     stock: Number(r["Stock"] || r["stock"] || 0),
//   })).filter((r) => r.giftCode);

//   // 2) Older Korean source workbook format
//   const koreanProductSheet = readSheet(wb, ["제품"]).map((r) => ({
//     brandName: String(r["브랜드 BRAND"] || "").trim(),
//     productImage: String(r["제품 이미지 PRODUCT IMAGE"] || "").trim(),
//     productName: String(r["제품명 PRODUCT NAME"] || "").trim(),
//     advantage: String(r["특  장  점 Advantage"] || "").trim(),
//     supplyPrice: Number(r["공급가 (vat별도)"] || 0),
//     consumerPrice: Number(r["소비자가"] || 0),
//     lowestPrice: Number(r["최저가"] || 0),
//     livePrice: Number(r["라이브 판매가"] || 0),
//     stock: 0,
//   }));

//   const koreanEventSheet = readSheet(wb, ["이벤트"]).map((r) => ({
//     eventCode: String(r["NO"] || "").trim(),
//     productName: String(r["제품명 PRODUCT NAME"] || "").trim(),
//     productImage: String(r["제품 이미지 PRODUCT IMAGE"] || "").trim(),
//     supplyPrice: Number(r["공급가 (vat 별도)"] || 0),
//     salePrice: Number(r["판매가"] || 0),
//     consumerPrice: Number(r["소비자가"] || 0),
//   }));

//   // 3) App-exported / legacy fallback
//   const catalogProductsSheet = readSheet(wb, ["CatalogProducts"]).map((r) => ({
//     brandCode: String(r["BrandCode"] || "").trim(),
//     brandName: String(r["BrandName"] || "").trim(),
//     productCode: String(r["ProductCode"] || "").trim(),
//     sku: String(r["SKU"] || "").trim(),
//     productName: String(r["ProductName"] || "").trim(),
//     productImage: String(r["ProductImage"] || "").trim(),
//     advantage: String(r["Advantage"] || "").trim(),
//     stock: Number(r["Stock"] || 0),
//     supplyPrice: Number(r["SupplyPrice"] || 0),
//     consumerPrice: Number(r["ConsumerPrice"] || 0),
//     lowestPrice: Number(r["LowestPrice"] || 0),
//     livePrice: Number(r["LivePrice"] || 0),
//     active: toBool(r["Active"], true),
//   }));

//   const catalogEventsSheet = readSheet(wb, ["CatalogEvents"]).map((r) => ({
//     eventSku: String(r["EventSKU"] || "").trim(),
//     eventCode: String(r["EventCode"] || "").trim(),
//     productName: String(r["ProductName"] || "").trim(),
//     productImage: String(r["ProductImage"] || "").trim(),
//     supplyPrice: Number(r["SupplyPrice"] || 0),
...
//   //   stock: Number(r.Stock || 0),
//   //   supplyPrice: Number(r.SupplyPrice || 0),
//   //   consumerPrice: Number(r.RetailPrice || 0),
//   //   livePrice: Number(r.OnlinePrice || 0),
//   // }));
// const legacyMain = getSheet("MainProducts").map((r) => ({
//   brandName: String(r.BrandName || "").trim(),
//   productName: String(r.ProductName || "").trim(),
//   productCode: String(r.ProductCode || "").trim(),
//   stock: Number(r.Stock || 0),
...
//     const normalized = generateCatalogProducts(productsSheet);
//     brands = normalized.brands;
//     catalogProducts = normalized.products;
...
//     catalogEvents = generateCatalogEvents(eventProductsSheet);
//   } else if (catalogEventsSheet.length > 0) {
//     catalogEvents = generateCatalogEvents(catalogEventsSheet);
//   } else if (koreanEventSheet.length > 0) {
//     catalogEvents = generateCatalogEvents(koreanEventSheet);
//   }

//   // Aliases: import explicit alias mapping if present, else auto-build
//   if (aliasMappingSheet.length > 0) {
//     aliasTable = uniqueBy(aliasMappingSheet, (a) => `${a.aliasName}__${a.targetSku}`);
//   } else {
